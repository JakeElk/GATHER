...
class Gather(Core.Model):
    ...
    default_resource_distribution = [0.7, 0.15, 0.1, 0.05]  # [0.0, 1.0, 0.0, 0.0]  #   change first ine back to 0.7
    ...
    def create_environment_resources(self, mode: int):
        ...
        elif mode == Gather.ENV_SHRUNK:
            from PIL import Image
            generator = np.asarray(Image.open('./resources/shrunk.png').convert('L')) / 255.0
            generator = generator.flatten()
            generator = np.where(generator > 0.0, 2, 1)

        elif mode == Gather.ENV_TEN:
            from PIL import Image
            generator = np.asarray(Image.open('./resources/10.png').convert('L')) / 255.0
            generator = generator.flatten()
            generator = np.where(generator > 0.0, 2, 1)
        
        elif mode == Gather.ENV_TWENTY:
            from PIL import Image
            generator = np.asarray(Image.open('./resources/20.png').convert('L')) / 255.0
            generator = generator.flatten()
            generator = np.where(generator > 0.0, 2, 1)

        elif mode == Gather.ENV_THIRTY:
            from PIL import Image
            generator = np.asarray(Image.open('./resources/30.png').convert('L')) / 255.0
            generator = generator.flatten()
            generator = np.where(generator > 0.0, 2, 1)

        elif mode == Gather.ENV_FOURTY:
            from PIL import Image
            generator = np.asarray(Image.open('./resources/40.png').convert('L')) / 255.0
            generator = generator.flatten()
            generator = np.where(generator > 0.0, 2, 1)

        elif mode == Gather.ENV_FIFTY:
            from PIL import Image
            generator = np.asarray(Image.open('./resources/50.png').convert('L')) / 255.0
            generator = generator.flatten()
            generator = np.where(generator > 0.0, 2, 1)

        elif mode == Gather.ENV_SIXTY:
            from PIL import Image
            generator = np.asarray(Image.open('./resources/60.png').convert('L')) / 255.0
            generator = generator.flatten()
            generator = np.where(generator > 0.0, 2, 1)

        elif mode == Gather.ENV_THIRTYTWO:
            from PIL import Image
            generator = np.asarray(Image.open('./resources/32.png').convert('L')) / 255.0
            generator = generator.flatten()
            generator = np.where(generator > 0.0, 2, 1)

        elif mode == Gather.ENV_THIRTYFOUR:
            from PIL import Image
            generator = np.asarray(Image.open('./resources/34.png').convert('L')) / 255.0
            generator = generator.flatten()
            generator = np.where(generator > 0.0, 2, 1)

        elif mode == Gather.ENV_THIRTYSIX:
            from PIL import Image
            generator = np.asarray(Image.open('./resources/36.png').convert('L')) / 255.0
            generator = generator.flatten()
            generator = np.where(generator > 0.0, 2, 1)

        elif mode == Gather.ENV_FOURTYTWO:
            from PIL import Image
            generator = np.asarray(Image.open('./resources/42.png').convert('L')) / 255.0
            generator = generator.flatten()
            generator = np.where(generator > 0.0, 2, 1)
